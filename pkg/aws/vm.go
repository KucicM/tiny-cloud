...
func NewVm(api VmAPI, req VmRequest) error {
	...
	for err == nil && state != Ready {
		...
		case Create:
			vmId, state, err = createVm(api, req.vmType, req.keyName, req.sgName)
...
func createVm(api VmAPI, vmType, keyName, sgName string) (string, VmState, error) {
